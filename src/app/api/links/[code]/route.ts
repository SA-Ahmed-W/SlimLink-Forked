...
export async function GET(req: Request, context: { params: Promise<{ code: string }> }) {
    try {
        ...
        if (error instanceof DatabaseError) {
            return NextResponse.json({ error: 'Failed to fetch URL', details: error.message }, { status: 500 })
        }
...
export async function PATCH(req: Request, context: { params: Promise<{ code: string }> }) {
    try {
        ...
        return NextResponse.json({ error: "No valid fields to update. Provide 'redirectUrl'" }, { status: 400 })
    } catch (error: unknown) {
        ...
        if (error instanceof DatabaseError) {
            return NextResponse.json({ error: 'Failed to update URL', details: error.message }, { status: 500 })
        }
...
export async function DELETE(req: Request, context: { params: Promise<{ code: string }> }) {
    try {
        ...
        if (error instanceof DatabaseError) {
            return NextResponse.json({ error: 'Failed to delete URL', details: error.message }, { status: 500 })
        }
